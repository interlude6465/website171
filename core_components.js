...
(function(window) {
    ...
    function patchPIN() {
      ...
      // Query the dots LIVE on every use. Capturing them once was fragile: if
      // the snapshot was taken before the dots existed, dots.length was 0 and
      // pressDigit's "buffer.length >= dots.length" guard returned on every key
      // (keys highlighted via CSS :active, but no dot ever filled).
      function getDots() { return Array.from(document.querySelectorAll('.pin-dot-fs')); }
      var keyButtons = Array.from(document.querySelectorAll('.key-btn-fs[data-key]'));
      var backBtn = document.getElementById('pinBackFS');
      var buffer = [];
      function getCurrentPIN() { return (typeof window.getAdminPIN === 'function') ? window.getAdminPIN() : '457511'; }
      function updateDots() { getDots().forEach(function(dot, i) { dot.classList.toggle('filled', i < buffer.length); }); }
      ...
      function pressDigit(d) {
        var n = getDots().length || 6;
        try { if (window.__dbg) window.__dbg('pressDigit(' + d + ') disp=' + pinOverlay.style.display + ' hidden=' + pinOverlay.classList.contains('pin-hidden') + ' buf=' + buffer.length + '/' + n); } catch (e) {}
        if (pinOverlay.style.display === 'none') return;
        if (pinOverlay.classList.contains('pin-hidden')) return;
        if (buffer.length >= n) return;
        buffer.push(d); updateDots();
        if (buffer.length === n) { setTimeout(tryUnlock, 100); }
      }
      function backspace() {
        if (pinOverlay.style.display === 'none') return;
        if (pinOverlay.classList.contains('pin-hidden')) return;
      ...
      try { if (window.__dbg) window.__dbg('patchPIN running: overlay=' + !!pinOverlay + ' keyButtons=' + keyButtons.length + ' dots=' + getDots().length); } catch (e) {}
