...
.modal-content{
  background:#fff; padding:16px; border-radius:8px; max-width:90%; width:300px; text-align:center;
}
.modal-content canvas{width:100%; height:150px; border:1px solid #ccc; border-radius:4px; background:#fafafa; touch-action:none;}
...
<div id="viewport">
  <div id="scroll-content">
    ...
    <div class="details" id="identity">
      ...
      <div class="address"><span class="licenceAddress">YOUR ADDRESS<br>HERE</span></div>
      <div class="divider"></div>
      <div class="sig"><div class="label">Signature</div><canvas class="sigCanvas"></canvas></div>
    </div>
...
<div id="editModal">
  <div class="modal-content">
    ...
    <input id="editDOB" class="edit-field" placeholder="Date of birth (e.g. 01 May 2000)" />
    <input id="editAddress" class="edit-field" placeholder="Address (make it random)" />
...
photoInput.addEventListener("change", e => {
  ...
  if(file){
    ...
    reader.onload = () => { document.getElementById("profilePhoto").src = reader.result; saveData(); };
...
editBtn.onclick = () => {
  ...
  document.getElementById("editDOB").value     = dobEl  ? dobEl.innerText : "";
...
saveEditBtn.onclick = () => {
  ...
  if(newDOB) { calculateDates(newDOB); }
  editModal.style.display = "none";
  saveData();
};

function calculateDates(dobString) {
  try {
    ...
    const birthMonth = months[monthStr];
    if(birthMonth === undefined) return;
    const currentYear      = new Date().getFullYear();
    const birthdayThisYear = new Date(currentYear, birthMonth, day);
    const maxIssueDate     = new Date(birthdayThisYear);
    maxIssueDate.setMonth(maxIssueDate.getMonth() + 2);
    const daysDiff   = Math.floor((maxIssueDate - birthdayThisYear) / (1000 * 60 * 60 * 24));
    const randomDays = Math.floor(Math.random() * (daysDiff + 1));
    const issueDate  = new Date(birthdayThisYear);
  ...
  } catch(e) { console.log("Date calculation error:", e); }
}
...
function loadData() {
  ...
  if(dob){  document.querySelectorAll(".licenceDOB").forEach(el     => el.innerText  = dob); calculateDates(dob); }
